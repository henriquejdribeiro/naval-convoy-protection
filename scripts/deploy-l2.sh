...
deploy_to() {
    ...
    #    as felt252.
    #
    #    l1_commander_addr is the L1 address that the L2 contract will
    #    accept as the authorised sender of #[l1_handler] open_mission
    #    messages. When Registry.deploy() on L1 fires sendMessageToL2,
    #    StarknetCoreStub records `msg.sender` (the calling contract,
    #    NOT the EOA that called Registry) as the `from_address`. So
    #    l1_commander_addr must equal the L1 Registry's address — NOT
    #    the commander's EOA, even though that EOA is the one that
    #    signed the Registry.deploy tx.
    #
    #    l1_verifier_addr is the L1 destination for L2 → L1 MissionSafe
    #    messages. send_message_to_l1_syscall uses this as its first
    #    arg, and L1's Verifier.consumeL2Message will be msg.sender of
...
EOF
    ...
    #    (Cairo 1 Map default). Confirms the contract responds to calls.
    #    Non-fatal: if it doesn't pass within ~30s the deploy is still
    #    recorded above.
    echo "[deploy-l2/${swarm}] smoke test: safe_count(0) (expect 0x0)..."
    ...
    echo "[deploy-l2/${swarm}] smoke test never passed — address may still be sealing"
