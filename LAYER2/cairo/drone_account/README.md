...
## Reproduce + verify (source → deployed class hash)
    ...
    docker build -t scarb-2.6.3 - < LAYER2/cairo/drone_account/Dockerfile.scarb-2.6.3
    docker run --rm -v "$(pwd):/work" -w /work/LAYER2/cairo/drone_account scarb-2.6.3 scarb build
    docker run --rm -v "$(pwd):/work" -w /work convoy-cairo-builder \
      starkli class-hash LAYER2/cairo/drone_account/target/dev/drone_account_AccountUpgradeable.contract_class.json
