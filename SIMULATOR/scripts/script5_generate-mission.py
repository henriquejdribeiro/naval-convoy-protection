...
in the schema `SIMULATOR/scripts/script6_submit-telemetry.sh` consumes:
...
Usage:
  python3 SIMULATOR/scripts/script5_generate-mission.py --scenario both-safe              --output-dir /tmp/sweeps/
  python3 SIMULATOR/scripts/script5_generate-mission.py --scenario both-unsafe            --output-dir /tmp/sweeps/
  python3 SIMULATOR/scripts/script5_generate-mission.py --scenario mixed                  --output-dir /tmp/sweeps/
  python3 SIMULATOR/scripts/script5_generate-mission.py --scenario alpha-dropout-vanish   --output-dir /tmp/sweeps/
  python3 SIMULATOR/scripts/script5_generate-mission.py --scenario alpha-dropout-midflight --output-dir /tmp/sweeps/
  python3 SIMULATOR/scripts/script5_generate-mission.py --scenario dual-dropout           --output-dir /tmp/sweeps/
...
Feeding generated files to submit-telemetry.sh:
  ...
  python3 SIMULATOR/scripts/script5_generate-mission.py --scenario both-safe
  for swarm in alpha bravo; do
      for did in 1 2 3 4 5; do
          f=.tmp-l2/missions/both-safe/${swarm}_${did}.json
          [ -f "$f" ] && ./SIMULATOR/scripts/script6_submit-telemetry.sh $swarm $did "$f"
      done
  done
"""
