...
#
# Wraps the per-drone SIMULATOR/scripts/submit-telemetry.sh in a loop over both swarms'
...
#
# Usage:
#   ./SIMULATOR/scripts/submit-all-telemetry.sh [scenario] [missions-dir]
...
if [ ! -d "${SCEN_DIR}" ]; then
    ...
    python3 SIMULATOR/scripts/generate-mission.py --scenario "${SCENARIO}" --output-dir "${MISSIONS_DIR}"
fi
...
for swarm in alpha bravo; do
    for did in 1 2 3 4 5; do
        ...
        if ! ./SIMULATOR/scripts/submit-telemetry.sh "${swarm}" "${did}" "${f}"; then
            echo "[submit-all] ${swarm}/${did} failed — retrying once..."
            ./SIMULATOR/scripts/submit-telemetry.sh "${swarm}" "${did}" "${f}" \
