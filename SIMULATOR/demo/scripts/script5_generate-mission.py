...
in the schema `SIMULATOR/demo/scripts/script6_submit-telemetry.sh` consumes:
...
Usage:
  python3 SIMULATOR/demo/scripts/script5_generate-mission.py --scenario both-safe              --output-dir /tmp/sweeps/
  python3 SIMULATOR/demo/scripts/script5_generate-mission.py --scenario both-unsafe            --output-dir /tmp/sweeps/
  python3 SIMULATOR/demo/scripts/script5_generate-mission.py --scenario mixed                  --output-dir /tmp/sweeps/
  python3 SIMULATOR/demo/scripts/script5_generate-mission.py --scenario alpha-dropout-vanish   --output-dir /tmp/sweeps/
  python3 SIMULATOR/demo/scripts/script5_generate-mission.py --scenario alpha-dropout-midflight --output-dir /tmp/sweeps/
  python3 SIMULATOR/demo/scripts/script5_generate-mission.py --scenario dual-dropout           --output-dir /tmp/sweeps/
...
Feeding generated files to submit-telemetry.sh:
  ...
  python3 SIMULATOR/demo/scripts/script5_generate-mission.py --scenario both-safe
  for swarm in alpha bravo; do
      for did in 1 2 3 4 5; do
          f=SIMULATOR/demo/.tmp-l2/missions/both-safe/${swarm}_${did}.json
          [ -f "$f" ] && ./SIMULATOR/demo/scripts/script6_submit-telemetry.sh $swarm $did "$f"
      done
  done
"""
...
def main() -> int:
    ...
    ap.add_argument(
        "--output-dir",
        type=Path,
        default=Path("SIMULATOR/demo/scenarios"),
        help="directory to write the per-drone input JSONs (default: SIMULATOR/demo/scenarios)",
    )
    ap.add_argument(
        "--seed",
        type=int,
